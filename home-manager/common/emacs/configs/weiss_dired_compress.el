(defun weiss-dired-unzip-with-password ()
  ...
  (let ((file (car (dired-get-marked-files nil nil nil nil t)))
    ...
    (dired-shell-command (format "unzip -P %s %s" pw file))
    (revert-buffer)))
...
(defun weiss-dired-do-compress-to-zip (&optional file)
  ...
  (interactive)
  (let* ((in-files
          (or (list file) (dired-get-marked-files nil nil nil nil t)))
         (out-file
          (if (= (length in-files) 1)
              (format "%s.zip"
                      (file-name-sans-extension (car in-files)))
            (concat
             (expand-file-name (read-string "zip name: "))
             ".zip")))
