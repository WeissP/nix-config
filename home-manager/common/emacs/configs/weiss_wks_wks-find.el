...
(defun wks--find-symbol (symbol forward)
  ...
  (interactive)
  (if forward
      (progn (search-forward symbol nil nil) (left-char))
    (progn (search-backward symbol nil nil) (right-char)))
  )
...
(defun wks-find-symbol-forward ()
  ...
  (interactive)
  (let ((symbol
         (wks--set-find-symbol t
                               (char-to-string (read-char "symbol: ")))))
