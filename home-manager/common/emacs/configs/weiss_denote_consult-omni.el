...
(defun weiss-denote-consult-omni-link ()
  ...
  (let ((consult-omni-multi-sources '("Denote fd link" "Denote heading link"))
        (initial-text (when (use-region-p) (s-downcase (buffer-substring-no-properties (region-beginning) (region-end)))))
        )
    (setq weiss-consult-omni-init-text initial-text)
    (consult-omni-multi initial-text)
    ))

(defun weiss-consult-omni-denote-heading-insert-link (cand)
  "DOCSTRING" 
  (interactive)
  (weiss-denote-org-link-to-heading
   nil
   cand
   (lambda (file-text heading-text) (or weiss-consult-omni-init-text heading-text)))
  )
...
(defun weiss-consult-omni-denote-insert-link (cand)
  ...
  (let* ((insert-fun (consult--read
                      '("denote-insert-link" "denote-org-link-to-heading")
    ...
    (pcase insert-fun
      ("denote-insert-link" (denote-link
                             cand 'org 
                             (or weiss-consult-omni-init-text (denote-query-extract-title cand))
                             ))
...
(defun consult-omni--denote-heading-link-fetching-results (input &rest args)
  ...
  (->>
   ...
   ((lambda (f) (ignore-errors (denote-org--get-outline f))))       
...
(defun weiss-denote-org-link-to-heading (file heading &optional heading-description)
  "DOCSTRING"
  (interactive)
  (let* ((heading-description (or heading-description #'denote-link-format-heading-description))
         ...
         (heading (or heading (denote-org-outline-prompt file)))
...
(with-eval-after-load 'consult-omni
  (with-eval-after-load 'weiss_denote_settings
    ...
    (defvar weiss-consult-omni-init-text "")
