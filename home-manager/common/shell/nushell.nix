...
with myEnv;
{
  ...
  programs.nushell =
    ...
    in
    {
      ...
      extraConfig = ''
        alias "trash" = gtrash put --home-fallback 
      ...
      configFile.text = ''
        ...
        def transfer [from_path: string, to_path: string] {
            rsync -PamAXvtu -e ssh $from_path weiss@${secrets.nodes.homeServer.localIp}:$to_path
        }
      '';
    };
}
